...
namespace MovenCore
{
    ...
    public class WMXMotion
    {
        ...
        public int ServoOn(int axisIndex)
        {
            int ret = -1;
            ret = m_coreMotion.AxisControl.SetServoOn(axisIndex, 1);
            return ret;
        }

        public int ServoOff(int axisIndex)
        {
            int ret = -1;
            ret = m_coreMotion.AxisControl.SetServoOn(axisIndex, 0);
            return ret;
        }
        ...
        public int JogMove(AxisProfile jogProfile)
        {
            ...
            jog.Profile.Type = (ProfileType)jogProfile.m_profileType;
            jog.Profile.Velocity = jogProfile.m_velocity;
            jog.Profile.Acc = jogProfile.m_acc;
            jog.Profile.Dec = jogProfile.m_dec;
            jog.Profile.JerkAccRatio = jog.Profile.JerkDecRatio = jogProfile.m_jerkRatio;

            ret = m_coreMotion.Motion.StartJog(jog);
            return ret;
        }

        public int JogMove(uint axisCount, AxisProfile[] jogProfiles)
        {
            int ret = -1;
            Motion.JogCommand[] jog = new Motion.JogCommand[axisCount];
            for (int i = 0; i < axisCount; i++)
            {
                jog[i] = new Motion.JogCommand();
            }

            for (int i = 0; i < axisCount; i++)
            {
                jog[i].Axis = jogProfiles[i].m_axis;
                jog[i].Profile.Type = (ProfileType)jogProfiles[i].m_profileType;
                jog[i].Profile.Velocity = jogProfiles[i].m_velocity;
                jog[i].Profile.Acc = jogProfiles[i].m_acc;
                jog[i].Profile.Dec = jogProfiles[i].m_dec;
                jog[i].Profile.JerkAccRatio = jog[i].Profile.JerkDecRatio = jogProfiles[i].m_jerkRatio;
            }
            ret = m_coreMotion.Motion.StartJog(axisCount, jog);
            return ret;
        }
        ...
        public int StopServo(int axisIndex)
        {
            int ret = -1;
            ret = m_coreMotion.Motion.Stop(axisIndex);
            return ret;
        }

        public int StopServo(int axisIndex, double dec) {
            int ret = -1;
            ret = m_coreMotion.Motion.Stop(axisIndex, dec);
            return ret;
        }
