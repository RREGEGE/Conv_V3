...
namespace Synustech
{
    internal class LongRect_Horizontal : CustomRectangle
    {
        private const string FIXED_TYPE = "Long_Horizontal";
        private const int FIXED_WIDTH = 80;  // Interface의 너비 고정값
        private const int FIXED_HEIGHT = 40; // Interface의 높이 고정값
        private const int FIXED_SENSOR_NUM = 3;

        public LongRect_Horizontal(int x, int y, int ID) : base(x, y, ID)
        {
        ...
        public override void AddSensorRectangle(int count)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                int childX = x + borderWidth + (i * spacing) +5;
                int childY = y + borderWidth; // 부모 사각형 아래에 자식 배치
