...
namespace Synustech
{
    public partial class UserLog : UserControl
    {
        ...
        private void UserLog_Load(object sender, EventArgs e)
        {
            ...
            UserMenu.del_ELogSender_UserMenu += User_ELogSender;
            UserConvControl_Re.del_ELogSender_UserConvControl += User_ELogSender;
            UserCycleTest.del_ELogSender_UserCycleTest += User_ELogSender;
            Conveyor.del_ELogSender_Conveyor += User_ELogSender;
            G_Var.del_ELogSender_Global += User_ELogSender;
        }

        private void User_ELogSender(enLogType eType1, enLogLevel eLevel, enLogTitle eTitle1, string strLog)
        {
            Log( eType1, eLevel, eTitle1, $"{strLog}");
        }
        private void Motion_ElogSender(enLogType eType1, enLogLevel eLevel, enLogTitle eTitle1, string strLog)
        {
            Log(eType1, eLevel, eTitle1, $"{strLog}");
        }
        private void Panel_Resize(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                // 라벨 폰트 사이즈 조절
                UIFunction.AdjustListBoxFontSize(listLog, initialFontSize_listLog, ratio);
            }
            finally
            {
